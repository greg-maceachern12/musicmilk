  ...
  return (
    <>
      ...
      <div className="fixed bottom-0 left-0 right-0 h-24">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-gray-950 via-gray-900/30 to-gray-900/50 backdrop-blur-xl border-t border-white/10 shadow-2xl" />
        ...
        <div className="container mx-auto my-auto h-full relative z-10">
          <div className="flex items-center h-full px-4 lg:px-6 gap-4 lg:gap-6">
            ...
            >
              <div className="flex-shrink-0 w-16 h-16 sm:w-20 sm:h-20 rounded-xl overflow-hidden bg-white/5 backdrop-blur-sm shadow-lg ring-1 ring-white/10">
                {currentMix?.cover_url ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center bg-white/5">
                    <Music className="w-6 h-6 sm:w-7 sm:h-7 text-white/20" />
                  </div>
                )}
              </div>

              <div className="min-w-0 flex-1">
                <h3 className="font-medium text-sm sm:text-base text-white truncate group-hover:text-white/90 transition-colors">
                  {currentMix?.title}
                </h3>
                {currentMix?.artist && (
                  <p className="text-xs sm:text-sm text-white/50 truncate mt-0.5 group-hover:text-white/70 transition-colors">
                    {currentMix.artist}
                  </p>
                )}
              </div>
            ...
            >
              <div className="w-12 h-12 rounded-lg overflow-hidden bg-white/5 backdrop-blur-sm shadow-lg ring-1 ring-white/10">
                {currentMix?.cover_url ? (
                  <Image
                    src={currentMix.cover_url}
                    alt={currentMix.title}
            ...
            <div className="flex-1 flex items-center gap-1 sm:gap-2 lg:gap-3 min-w-0">
              ...
              <button
                ...
                className="flex-shrink-0 p-1 sm:p-2 rounded-full hover:bg-white/10 transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
              ...
              <button
                ...
                className="flex-shrink-0 p-2 sm:p-2.5 lg:p-3 rounded-full bg-white text-black hover:bg-white/90 transition-all duration-300 shadow-lg hover:shadow-[0_0_30px_rgba(255,255,255,0.12)] hover:scale-[1.02] active:scale-[0.98]"
                aria-label={isPlaying ? "Pause" : "Play"}
              >
                {isPlaying ? (
                  <Pause className="w-4 h-4 lg:w-5 lg:h-5 text-black fill-black" />
                ) : (
                  <Play className="w-4 h-4 lg:w-5 lg:h-5 text-black fill-black" />
                )}
              </button>
